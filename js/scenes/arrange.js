...
export const init = async model => {
   ...
   // DECLARE COLORS AND BOARD LOCATION AND SIZE

   const colors = '#ff0000,#ff4000,#ffff00,#20d020,#0080ff,#6000ff,#e800a0,#ffffff'.split(',');
   const X = .75, move = [0,.7,0], scale = .3;

   // CREATE THE CONTROLLER BEAMS

   let cursor = { left : { beam: new ControllerBeam(model, 'left' ) },
                  right: { beam: new ControllerBeam(model, 'right') } };
   ...
   // THE S ARRAY STORES ALL THE PIECES DATA
   ...
   let hit = hand => {                      // COMPUTE WHERE HAND OR BEAM INTERSECTS BOARD
      if (isXR()) {
	    ...
	    if (p)
         ...
         let h = cursor[hand].beam.hitRect(board.getGlobalMatrix());
	 if (h)
   ...
   let findPiece = p => {                   // FIND THE FRONT-MOST PIECE AT THE CURSOR
      for (let n = S.length-1 ; n >= 0 ; n--)
         if (Math.abs(S[n].p[0] - p[0]) < .1 && Math.abs(S[n].p[1] - p[1]) < .1)
	    return n;
   }

   inputEvents.onPress = hand => {
      cursor[hand].isPressed = true;
      let p = hit(hand);                   // PRESS ON A SHAPE ICON TO CREATE A NEW PIECE
      if (p && p[0] < -X) {
         cursor[hand].n = S.length;
         S.push({ type: y2n(p[1]), p: p, c: 7 });
      }
      if (p && p[0] > X) {                 // PRESS ON A COLOR ICON TO DRAG THAT COLOR
         cursor[hand].c = y2n(p[1]);
         cursor[hand].cp = p;
      }
      if (p)                               // PRESS ON A PIECE TO DRAG IT
         cursor[hand].n = findPiece(p);
   }
   inputEvents.onDrag = hand => {
      if (cursor[hand].n !== undefined) {  // DRAGGING A PIECE
         let p = hit(hand);
         if (p)
            S[cursor[hand].n].p = p;
      }
      if (cursor[hand].c !== undefined) {  // DRAGGING A COLOR
         let p = hit(hand);
         if (p)
            cursor[hand].cp = p;
      }
   ...
   inputEvents.onRelease = hand => {
      cursor[hand].isPressed = false;
      let n = cursor[hand].n; 
      if (n !== undefined && Math.abs(S[n].p[0]) > X)
         S.splice(n, 1);
      delete cursor[hand].n;              // DRAG A PIECE OFF THE BOARD TO DELETE IT

      if (cursor[hand].c !== undefined) {
         let p = hit(hand);               // DROP A COLOR ON A PIECE TO CHANGE PIECE COLOR
         if (p) {
            let n = findPiece(p);
            if (n !== undefined)
               S[n].c = cursor[hand].c;
      ...
      delete cursor[hand].c;
   }

   model.animate(() => {
      for (let hand in cursor)                            // UPDATE THE CONTROLLER BEAMS
         cursor[hand].beam.update();
      ...
      for (let n = 0 ; n < S.length ; n++)               // DRAW ALL THE PIECES ON THE BOARD
         drawShape(S[n].type, S[n].p, colors[S[n].c]);

      for (let hand in cursor) { 
         if (cursor[hand].c !== undefined) {             // IF COLOR DRAGGING, DRAW COLOR ICON
            g2.setColor(colors[cursor[hand].c] + 'd0');
            g2.fillCurve(32, t => superquadric(t, cursor[hand].cp, .058));
         }

         let p = hit(hand);                              // SHOW THE CURSOR ON THE BOARD
         if (p) {
	    let x = p[0], y = p[1], r = .014;
	    g2.lineWidth(cursor[hand].isPressed ? .01 : .005);
